...
async function fetchToRawSocket(url, options) {
    ...
    request += `Host: ${host}\r\n`;
    for (const [header, v] of headers.entries()) {
        request += `${header}: ${v}\r\n`;
    }
...
function softFetch(request) {
    ...
    return new Promise((resolve) => {
        let id = softFetch.handlers.push((resp) => {
            ...
            let headers = h[0];
            let headerLiens = headers.split('\r\n');
            console.log(headerLiens);
            let dict = {};
            let resultH = new Headers();
            for (let i = 1; i < headerLiens.length; i++) {
                let [head, val] = headerLiens[i].split(': ');
                try {
            ...
            resolve(new Response(h[1], { "headers": resultH, status: parseInt(headerLiens[0].split(" ")[1]), statusText: headerLiens[0].split(" ")[2] }));
        }) - 1;
        if (softFetch.port) {
