...
import archive from 'archive-wasm';
import createMemoryContext from './filesystem.tsx';
...
declare global {
  ...
  module globalThis {
    ...
    var memoryContextSettings: ReturnType<typeof import('./filesystem.jsx').default>;
...
function parentTillTarget(p: string, dira: (path: string) => void) {
  memoryContextSettings.fs.stat(p, (err, st) => {

    let parent = ps.resolve(p, '..');
    if (st.isDirectory()) {
      dira(p);
    }
    else {
      parentTillTarget(parent, dira);
    }
  })
}
...
function uploadFile() {
  ...
  parentTillTarget(select.selected.getAttribute('data-path'), function (str) {
    ...
    inp.onchange = async () => {
      if (inp.files && inp.files.length > 0) {
        ...
        inp.onchange = null;
        try {
          let ab = await a.arrayBuffer();
          let tPaht = ps.resolve(str, a.name);
          memoryContextSettings.fs.open(tPaht, "w+", (err, fd) => {
            console.log(Buffer.from(ab));
            if (err) {
              console.error("Failed at open" + err);
              
            } else {
              memoryContextSettings.fs.write(fd, Buffer.from(ab), 0, ab.byteLength, 0, (err, written, buffer) => {
                if (err) {
                  console.log("failed at write");
                  return;
                }
                console.log(written);
              })
...
contextMenuState.addSection("upload-file", "Upload file", (ev: PointerEvent) => {
  uploadFile();
...
const editorContext = {
  ...
  set fs(fs: typeof import('fs')) {
    this._fs = fs;
    fs.watch("/", { recursive: true }, function (evt, filename) {
      fs.stat(filename, (err, stat) => {
        if (err) {
          if (err.code === "ENOENT") {
            return;
          }
          console.error("While detecting changes, an error has been encountered while accessing " + filename + " which was a " + err.code);
          return;
        }
        if (stat.isDirectory())
...
async function downloadToBuffer(param, writeFunc: (loaded: number, total: number) => void, toSave) {
  ...
  let wasmMemory = new WebAssembly.Memory({ initial: Math.ceil((3 * 1024 * 1024 * 1024) / 65536) });
  wasmMemory.cursor = 0;
...
function showRename(path, switchTo = false) {
  ...
  inp.onkeydown = async (ev) => {
    console.log("got here too quick?")
  ...
  if (count === 0) {
    ...
    while (true) {
      ...
      if (c.done) {
        ...
        let tl = 0;
        for (let bufs of chunks) {
          if (!bufs) {
            continue;
          }
          new Uint8Array(totalWasmMemory.buffer).set(bufs, tl);
...
function XTermComponent() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      globalThis.memoryContextSettings = createMemoryContext(fs, select, editorContext);
      editorContext.fs = memoryContextSettings.fs;
      fs = memoryContextSettings.fs;

      memoryContextSettings.reference = reference;
      ...
      const emulator = new V86.V86({
        ...
        filesystem: memoryContextSettings.vmObject,
  ...
  useEffect(() => {
    if (('memoryContextSettings') in globalThis)
      memoryContextSettings.invalidationCB();
  }, [reference]);
    ...
    <div style={{ minWidth: "10%" }}>
      ...
      <div style={{ height: "100%" }}>
        {Object.values(panels).map((a) => {
          ...
          return (
            <div ref={a.ref} key={a.name} style={{ height: "100%" }}>
              {rElem}
            </div>
          )
        })}
